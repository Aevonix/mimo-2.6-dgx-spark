...
thread_local std::vector<int64_t> last_config;
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* b_bias,
               ...
               bool use_fp32_reduce, bool is_zp_float, bool whole_k) {
  ...
  last_config = {thread_k, thread_n, num_threads, exec_cfg.blocks_per_sm,
                 sms, moe_block_size, prob_m, prob_n, prob_k};
  ...
  kernel<<<blocks, num_threads, max_shared_mem, stream>>>(
      ...
      has_bias, use_atomic_add, use_fp32_reduce, whole_k);
  // clang-format on
}
...
torch::stable::Tensor moe_wna16_marlin_gemm(
    ...
    int64_t blocks_per_sm, bool whole_k) {
  ...
  MARLIN_NAMESPACE_NAME::marlin_mm(
      ...
      use_atomic_add, use_fp32_reduce, is_zp_float, whole_k);

  return c;
}

STABLE_TORCH_LIBRARY_IMPL(_mimo_marlin_whole_k_v1, CUDA, m) {
  m.impl("moe_wna16_marlin_gemm", TORCH_BOX(&moe_wna16_marlin_gemm));
}

STABLE_TORCH_LIBRARY_FRAGMENT(_mimo_marlin_whole_k_v1, m) {
  m.def(
      ...
      "int thread_k, int thread_n, int blocks_per_sm, bool whole_k=False) -> Tensor");
  m.def("last_config() -> int[]");
}

std::vector<int64_t> mimo_last_config() { return MARLIN_NAMESPACE_NAME::last_config; }
STABLE_TORCH_LIBRARY_IMPL(_mimo_marlin_whole_k_v1, CompositeExplicitAutograd, m) {
  m.impl("last_config", TORCH_BOX(&mimo_last_config));
}
