...
BASE_SHA = "23deadbe773b27106afd1dcb4c6d73ac6219c6ae9373fdd8e4ea316defe3baf1"
...
def main(args):
    ...
    import sys
    import importlib
    os.environ["MIMO_WHOLE_K_LIBRARY"] = str(Path(args.whole_k_library).resolve())
    package_name = "vllm.model_executor.layers.fused_moe.experts"
    importlib.import_module(package_name)
    def load_child(name, filename):
        child_name = package_name + "." + name
        child_spec = importlib.util.spec_from_file_location(child_name, Path(args.integration_dir) / filename)
        child = importlib.util.module_from_spec(child_spec)
        sys.modules[child_name] = child
        child_spec.loader.exec_module(child)
        return child
    shim = load_child("_mimo_whole_k", "_mimo_whole_k.py")
    parent = load_child("_mimo_whole_k_parent_smoke", "marlin_moe.py")
    assert parent._mimo_whole_k_gemm is shim.moe_wna16_marlin_gemm
    invoked_shim = parent._mimo_whole_k_gemm
    ...
    def emit(record):
        ...
        with (output / "parent-w13-results.jsonl").open("a") as stream:
            stream.write(line + "\n")
    ...
    def witness(*call_args, **kw):
        ...
        valid = active[(active >= 0) & (active < m * kw["top_k"])].long().unique(sorted=True)
        ...
        if label == "canonical_0":
            compile_args = list(call_args)
            compile_args[1] = torch.zeros_like(original)
            from torch._subclasses.fake_tensor import FakeTensorMode
            mode = FakeTensorMode()
            with mode:
                fake_args = [mode.from_tensor(x) if isinstance(x, torch.Tensor) else x for x in compile_args]
                fake_result = invoked_shim(*fake_args, **kw)
                assert tuple(fake_result.shape) == (kw["size_m"] * kw["top_k"], kw["size_n"])
                assert fake_result.dtype == call_args[0].dtype
                assert fake_result.device == call_args[0].device
            compiled_result = invoked_shim(*compile_args, **kw)
            comparison = compare(fixed[valid], compiled_result[valid])
            emit({"event": "actual_parent_fake_and_eager", "m": m,
                  "comparison": comparison, "direct_fake_metadata": True, "inner_inductor_tested": False,
                  "custom_op": "_mimo_marlin_whole_k_v1::moe_wna16_marlin_gemm"})
            assert comparison["byte_equal"], "parent shim differs from explicitly enabled operation"
            del compiled_result, compile_args
            canonical_configs[m] = fixed_config
            input_hashes[m] = digest(call_args[0][0])
            torch.save({"activation_row0": call_args[0][0].detach().cpu(),
            ...
            if m in (8, 1498):
                ...
                graph_args = list(call_args)
                graph_args[1] = fixed
                warm_stream = torch.cuda.Stream(); warm_stream.wait_stream(torch.cuda.current_stream())
                with torch.cuda.stream(warm_stream): invoked_shim(*graph_args, **kw)
                torch.cuda.current_stream().wait_stream(warm_stream)
                graph = torch.cuda.CUDAGraph()
                with torch.cuda.graph(graph): graph_output = invoked_shim(*graph_args, **kw)
    ...
    emit({"event": "whole_k_complete", "scope": "synthetic W13 kernel witness; W2 and full-model causation remain unproven"})
